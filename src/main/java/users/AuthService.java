...
public class AuthService implements AuthInterface{
    ...
    public User login(String username, String password) throws SQLException{
        ...
        String sql = String.format("SELECT codigo, usuario, correo, contrasena FROM login WHERE usuario='%s' AND contrasena='%s'", username, password);
        // Ejecución de la consulta
        ResultSet querySet = statement.executeQuery(sql);
        // Recorrido del resultado de la consulta
        if(querySet.next()) {
            //Obtenemos los datos del Individuo
            long id = querySet.getLong("codigo");
            String usuario = querySet.getString("usuario");
            String email = querySet.getString("correo");
            String contrasena = querySet.getString("contrasena");
            result = new User(id, usuario, email, contrasena);
        }
    ...
    public User register(String username, String email, String password) throws SQLException {
         ...
         Statement statement = this.conn.createStatement();    
         String sql = String.format("INSERT INTO login (usuario, correo, contrasena) VALUES ('%s', '%s', '%s')",username, email, password);
         // Ejecución de la consulta
